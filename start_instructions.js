...
// Returns an ARRAY of separate survey-likert trials, one per item, so each
// item is saved as its own row in the data (instead of one trial bundling
// all 5 items into a single JSON blob). Spread this into a timeline with
// `...createLikertQuestions(...)`.
function createLikertQuestions(conditionName, items) {
  return items.map(function(item, idx) {
    return {
      type: jsPsychSurveyLikert,

      // Only show the intro text once, above the first item.
      preamble: idx === 0 ? `
...
const condition1ManipulationInstructions = {
  ...
  pages: [
    `
      <div style="max-width: 800px; margin: 0 auto; text-align: left;">
        <p>
          A long line of research has provided insights into the factors
          that influence people's ability to recognize facial
          microexpressions. The findings suggest that the ability to read
          microexpressions and recognize subtle emotional expressions
          depends on a person's emotional intelligence. Emotional
          intelligence is typically measured by an EQ score and people
          differ substantially in their level of emotional intelligence.
        </p>

...
// Last-resort fallback: if every DataPipe attempt failed, trigger a
// local download of the CSV so the data isn't lost outright.
function downloadDataLocally(filename, dataString) {
  try {
    ...
    console.warn("DataPipe save failed after all retries - triggered local CSV download as a fallback: " + filename);
  } catch (err) {
    console.error("Local fallback download also failed:", err);
  }
}
...
/* =======================================================
   ...
   blocks or slows down the task itself. Each call POSTs the
   FULL cumulative CSV so far to the SAME filename, which
   simply overwrites the previous (smaller) snapshot on OSF -
   so even if a later call fails, whatever was saved by the
   last successful call is preserved.
======================================================= */

window.__saveProgressSnapshot = function () {
...
const saving_and_redirect = {
  ...
  on_load: function () {
    ...
    saveToDataPipeWithRetries(filename, dataString).then(function (saved) {
      if (!saved) {
        downloadDataLocally(filename, dataString);
      }
      ...
      if (statusEl) {
        statusEl.innerHTML =
          "<p>" + (saved ? "Your data has been saved." : "Your data could not be uploaded, so it was saved as a local backup file instead.") + "</p>" +

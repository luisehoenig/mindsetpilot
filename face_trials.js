...
const ITEM_QUESTIONS = [
  {
    text: "Based on your first intuition, how high do you think your success rate could have been so far?",
    name: "expected_success_rate_so_far"
  },
  {
    text: "Now think about the next 10 faces of the task. How high do you think your success rate will be for the next 10 faces?",
    name: "expected_success_rate_next10"
  }
];
...
window.buildFaceTrials = function (jsPsych, imageList) {
  ...
  for (let i = 0; i < nTrials; i++) {
    ...
    // slider. Response for each is stored as an index 0-10 under its
    // named key (expected_success_rate_so_far / expected_success_rate_next10).
    if (trialNumber % FACE_TRIALS_PER_BLOCK === 0) {
      const blockNumber = trialNumber / FACE_TRIALS_PER_BLOCK;
      ITEM_QUESTIONS.forEach(function (question, qIdx) {
        const isLastQuestionOfBlock = qIdx === ITEM_QUESTIONS.length - 1;
        trials.push({
          type: jsPsychSurveyLikert,
          questions: [
            {
              prompt: '<p style="font-size:20px;">' + question.text + '</p>',
              name: question.name,
          ...
          // Point 1 (robustness fix): after the last question of each
          // block, kick off a background save of all data collected so
          // far, so a crash/network loss later in the task doesn't wipe
          // out everything. Fire-and-forget - defined in
          // experiment_mindset.js and exposed on window - so it never
          // delays or blocks the task itself. Each block gets its own
          // filename (see experiment_mindset.js) since DataPipe locks a
          // filename after its first submission.
          on_finish: isLastQuestionOfBlock
            ? function () {
                if (typeof window.__saveProgressSnapshot === "function") {
                  window.__saveProgressSnapshot(blockNumber);
                }
              }
            : undefined
        });

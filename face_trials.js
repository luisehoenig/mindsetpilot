/* face_trials.js
   ...
   IMAGE POSITION: the response trial and the feedback trial both wrap the
   image in an identical outer container (same top padding), and the
   response trial reserves an invisible spacer of the same height as the
   feedback trial's text line. This keeps the image pinned to the same
   fixed pixel position on screen across both trials, instead of visibly
   jumping down when the feedback text appears above it.
...
/* ---------------- fix: prevent image from jumping between response/feedback ----------------
   By default, jsPsych's core CSS vertically CENTERS the whole content block
   inside the display element (.jspsych-content-wrapper / .jspsych-content
   use display:table + vertical-align:middle). The response trial (image +
   spacer + prompt + response buttons) and the feedback trial (image +
   label only) have different total content heights, so centering shifts
   where that content sits on screen - which makes the face image appear to
   jump up/down, even though the image itself is always placed at the same
   padding-top offset within its own container.

   The padding-top + invisible-spacer approach already built into this file
   only produces a fixed pixel position if the container is TOP-aligned
   instead of vertically centered. This override is scoped to the
   `.face-trial-top-align` class (added via css_classes below) so it only
   applies to the response/feedback trials - every other trial in the
   experiment (consent, instructions, likert questions, etc.) keeps the
   default vertical centering.
------------------------------------------------------------------------- */
(function () {
  const style = document.createElement("style");
  style.textContent = `
    .jspsych-display-element.face-trial-top-align .jspsych-content-wrapper {
      display: block !important;
      position: static !important;
      height: auto !important;
    }
    .jspsych-display-element.face-trial-top-align .jspsych-content {
      display: block !important;
  ...
  for (let i = 0; i < nTrials; i++) {
    ...
    trials.push({
      ...
      stimulus: (
        ...
        'justify-content:flex-start; padding-top:' + FEEDBACK_TOP_PADDING_PX + 'px;">' +
      ...
      css_classes: ["face-trial-top-align"],
